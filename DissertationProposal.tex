...
\end{itemize}
On a scale where 1=``Very Difficult'' and 7=``Very Easy'', the mean values for the answers to these questions were 2.74, 3.46, 2.35, and 2.57, respectively.   Responses to a fifth question, ÒHow difficult is it for you to (mentally) parse Coq syntax?Ó, produced a mean value of 5.02 on the same scale, with only 4 responses indicating ÒDifficultÓ or ÒSomewhat difficultÓ.  Again, this may have been an artifact of the way the question was asked--how difficult, compared to what?
...
\begin{figure} [H]
  \center
  \includegraphics[width=.80\linewidth]{ErrorDataFixed.jpg}
...
\begin{figure} [H]
  \center
  \includegraphics[width=.80\linewidth]{ErrorRateDifferences.jpg}
...
\begin{figure} [H]
  \center
  \includegraphics[width=.80\linewidth]{OverallAverageTimeDifferences.jpg}
...
Like Proof Transitions, the ``{\bf Proposition Logic Syntax Shortcuts}'' CoqEdit extension combines two ideas.  The first idea is keyboard-card menus, described above. One reason I would like to develop Propositional Logic Syntax Shortcuts is to see if keyboard-card menus can be successfully integrated into a real application.  In the Propositional Logic Syntax Shortcut the user would most likely see a keyboard-card menu displayed in a dockable window, docked under the text area displaying the buffer. 

The second idea incorporated into Propositional Logic Syntax Shortcuts is ``{\bf Syntax Tree Highlighting}''.  Syntax Tree Highlighting  is text highlighting of parse-able text documents (e.g. programs) where
\begin{itemize}
\item only text of a node in the concrete syntax tree of the document is highlighted
...
\item the user can navigate the syntax tree  by moving the selection indicator from non-leaf node to non-leaf node and by moving the highlighting to the selected non-leaf child node or the parent of the node with highlighting.
\end{itemize}
Syntax Tree Highlighting could be useful in understanding the syntactic structure of a document (which, of course, is generally a prerequisite for understanding its semantics), and has an advantage, over other tree visualizations of syntactic structure, in that it is directly integrated into the display of the buffer.\footnote{With other visualizations, the user may have to switch his gaze back and forth between the text and the visualization}  It may be particularly helpful in resolving ambiguities that result when the author of the text did not include parentheses.  Ambiguities of this sort can occur in in many languages, which suggests that Syntax Tree Highlighting may be widely useful.  A good example is a math problem that, according to an article in \textit{Slate} magazine\cite{HaelleMarch122013}, has been circulating on \textit{Facebook}.  The problem is to compute
\begin{equation}
6 \div 2(1 + 2)
\end{equation}
...
\begin{figure} [H]
  ...
  \caption{Syntax Tree Highlighting for Propositional Logic Syntax Shortcuts, after deleting the text highlighted in \autoref{fig:plss2}}
  \label{fig:plss3}
\end{figure}
...
Testing the Proof Previews and Proof Transitions extensions with human participants will be combined in a single experiment, already approved by the University of Iowa Institutional Review Board (i.e. human subjects review board).  As part of this process, some informal feedback from Coq users and potential users, indicating that our Proof Previews and Proof Transitions ideas seem reasonable, has already been acquired. \footnote{For the now at least, I am considering the testing of Propositional Logic Syntax Shortcuts with human subject to be beyond the scope of the proposed dissertation.  However, given that it will be developed with Huth and Ryan's textbook in mind (which has been used at the University of Iowa, among other places), the plugin should certainly lend itself to testing in a classroom setting.}  The following briefly summarizes the experiment\footnote{Most of this section comes directly from our IRB approval petition}.

\paragraph{Research Goal}  Our research goal is to demonstrate the effectiveness of the Proof Transitions and Proof Previews CoqEdit extensions in helping novice Coq users to understand proofs others have written and to write their own proofs.

\paragraph{Materials} Participants will be interacting with Coq, jEdit, CoqEdit, Proof Transitions and Proof Previews on a Mac OS X system in a reserved room in the University of Iowa's MacLean Hall.  The simple examples and exercises they will be working through will come from the textbook \textit{Interactive Theorem Proving and Program Development: Coq'Art: The Calculus of Inductive Constructions} by Yves Bertot and Pierre Cast\'{e}ran \cite{bertot2004interactive}.

\paragraph{Participants} 16 participants will be recruited from amongst English-speaking students who have taken and passed 22c:019 Discrete Math and 22c:021 Data Structures at the University of Iowa, or equivalent courses (they will be asked if they have or not in the recruitment process).  They will be recruited from senior-level undergraduate computer science courses.  Participants will be emailed a consent document and asked to contact a member of the research team with any questions.  Regardless of whether they have any, the research team will contact them by phone and ask them if they have read the consent information.  If they answer no, the call will be rescheduled.  If they answer yes, the research team member will ask if they have any questions about the study, answer any they have, and ask if they agree to participate.  Participants will be compensated with \$40.

\paragraph{Procedures and Design} Participants will:
\begin{enumerate}
\item Fill out a demographic questionnaire
\item Go through a training module on interactive theorem proving with Coq.  This will take about 45 minutes.  The material covered should be accessible to students with the required backgrounds. 
\item Take a 10 minute break
\item Explain four simple theorem proofs, two using the basic CoqEdit interface, and two using CoqEdit+Proof Transitions. The sets of proofs will be equivalent in complexity. The proofs will be either directly from \cite{bertot2004interactive}, or will be variations of theorems presented in that book. The order of the proofs and the user interfaces will be counterbalanced across participants. They will have up to 15 minutes to work with each user interface. During this part of the study, audio and video of their faces will be recorded. Video and audio recording is mandatory.
\item Fill out the first half of a satisfaction and preferences questionnaire.
\item Take a 5 minute break.
\item Prove simple theorems. Participants will use two user interfaces again, one the basic CoqEdit, and the other CoqEdit+Proof Previews. They will try to prove as many theorems as possible in 15 minutes with each user interface. They will be given lists of 10 theorems to solve with each user interface. The two lists will be of the same difficulty. The theorems will, again, be either directly from  \cite{bertot2004interactive}, or will be variations of theorems presented in that book. The order of the sets of proofs and user interfaces will again be counterbalanced. 
\item Fill out remainder of the satisfaction and preferences questionnaire.
\end{enumerate}

\paragraph{Experimental Analysis} In addition to comparing the number of correctly proved theorems in the second-to-last step above and looking for differences in answers to the questionnaire, we will be asking a Coq user to grade the explanations given of theorems seen with and without the Proof Transitions extension without knowing which explanations were given with the extension.
...
\begin{itemize}
\item Completion of the revised version of CoqEdit, the Proof Previews plugin, and the Proof Transitions plugin by the end of August 2013.
...
\item Further refinement of plugins, dissertation writing, release plugins/source code/documentation publicly with open source license, February through April 2014
