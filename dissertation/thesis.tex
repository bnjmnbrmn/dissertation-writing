...
% example
\documentclass[phd,appendix,figures,tables,abstractpage]{uithesis}
...
\usepackage{amssymb}
\usepackage{epstopdf}
\usepackage{hyperref} %Note the \PassOptionsToPackage at top
\usepackage{float}
...
\author{Benjamin Alexander Berman}
...
\memberTwo{Aaron Stump}
\memberThree{Cesare Tinelli}
\memberFour{Alberto Segre}
\memberFive{Paul Muhly}
...
To try to evaluate all sentences in the buffer, the user can use the ``Go to end'' menu item.  This of course may not always be successful since non-terminating and error sentences may be encountered, as in \autoref{fig:coqeditpurple} and \autoref{fig:coqeditred}.  Note that when an error is processed, processing stops, the sentences-to-process queue is emptied, and the highlighting is updated to reflect this.  Users are thus informed of incorrect proof attempts.  
...
\begin{figure}[H]
  ...
  \caption{Proof Transitions zoomed-in view.}
...
As shown, the visualization moves nodes so as to keep the current node above its parent (so that both the current node and its parent are visible when one zooms in).  To try to keep the shape of the tree relatively stable (i.e. to help users know which nodes are the same in \autoref{fig:ptfirstchildzoomedout}, \autoref{fig:ptsecondchildzoomedout}, \autoref{fig:ptthirdchildzoomedout}, \autoref{fig:ptfourthgrandchildzoomedout}, for instance), the subtrees under the siblings, ``uncles'', ``great uncles'', etc., are each moved as a unit with each subtree getting its own allotment of space horizontally, \textit{and}, except for the parent of the current node, each parent node is kept centered above its left-most child (moving to the parent node from a child node other than the left-most requires shifting the subtree under the parent to the right to maintain this property).   
...
\begin{figure}[H]
  ...
  \caption{Proof Transitions zoomed-out view.}
...
\begin{figure}[H]
  ...
  \caption{Proof Transitions after moving to a second child node.}
...
\begin{figure}[H]
  ...
  \caption{Proof Transitions after moving to a third child node.}
...
\begin{figure}[H]
  ...
  \caption{Proof Transitions after moving to a grandchild node.}
...
\begin{figure}[H]
  ...
  \caption{Proof Transitions node with a purple ``visited'' marking.}
...
I conducted a user study with 16 participants that compared using 1) Proof Transitions with the basic CoqEdit interface for reading/interpreting proofs, and 2) Proof Previews with the basic CoqEdit interface for writing proofs.  The main comparison was within-subjects (each participant used both the basic and experimental interfaces) and counterbalanced (i.e. equal numbers of participants used each task-ordering/interface-ordering combination), and each participant did the Proof Transitions comparison followed by the Proof Previews comparison.  An additional between-subjects independent variable was the set of instructions given to participants, which was refined for half of the participants, as described below.  The dependent variables were, for the Proof Transitions portion of the experiment, the fraction of opportunities taken to point out where a branch splits as the result of a destruct tactic and, for the Proof Previews portion, the number of proofs written (in a maximum of 15 minutes) and the amount of time actually taken (to attempt to write 10 proofs).  To summarize, each participant had one session with two parts:

\begin{itemize}
	\item Part one:  Proof Transitions' effect on proof interpretation
	\begin{itemize}
		\item Independent variables:
			\begin{itemize}
				\item Proof Transitions vs. basic CoqEdit (within-subjects).
				\item Refined vs. unrefined instructions as described below (between-subjects).
				\item Task-ordering/interface ordering combination (between-subjects)
			\end{itemize}
		\item Dependent variables:
			\begin{itemize}
				\item Fraction of opportunities taken to point out where a branch splits as the result of a destruct tactic
			\end{itemize}
	\end{itemize}
	\item Part two:  Proof Previews' effect on proof writing
	\begin{itemize}
		\item Independent variables:
			\begin{itemize}
				\item Proof Previews vs. basic CoqEdit (within-subjects).
				\item Refined vs. unrefined instructions as described below (between-subjects).  Note that under the refined instructions, participants were required to use the Proof Previews suggestion feature exclusively when it was available.
				\item Task-ordering/interface ordering combination (between-subjects)
			\end{itemize}
		\item Dependent variables:
			\begin{itemize}
				\item Number of (correct) proofs written (capped at 10)
				\item Amount of time taken to write a set 10 of proofs (capped at 15 minutes)
			\end{itemize}
	\end{itemize}
\end{itemize}
...
\item Before using Proof Previews, participants were walked through how to use the extension to create a three tactic proof of A \ra (B \ra C) \ra (A \ra C) \ra C.  \textbf{They were also told to use Proof Previews exclusively}.
\end{itemize}
...
Proof Transitions and Proof Previews each had benefits according to relatively \textbf{objective measures}.  For Proof Transitions, the dependent variable I found easiest to measure was \textit{opportunities taken to explicitly make the observation that a destruct tactic produced two child subgoals}.  Each participant had six opportunities to make this observation, three with Proof Transitions and three without.  With Proof Transitions, users were significantly more likely to make this observation when the opportunity presented itself---\textbf{33 out of 41 opportunities were taken with Proof Transitions vs 17 out of 38 without} (a repeated measures ANOVA produced p=.023).  (Some participants ran out of time, or their explanations were inaudible at these points, so the total numbers of opportunities for the 16 participants was reduced from 48 to 41 and 38 for explanations with and without Proof Transitions respectively).

\begin{figure}[H]
  \center
  \includegraphics[width=.8\linewidth]{PTSplits.jpg}
  \caption{The fraction of opportunities that each participant took, with and without the Proof Transitions visualization, to point out that a tactic produced two child subgoals, rather than one.  (The number of opportunities was reduced from 3 when a participant ran out of time or their description of the relevant part of the proof was inaudible).}
  \label{fig:ptsplits}
\end{figure}

Timing results from the Proof Previews part of the experiment are shown in \autoref{fig:minuteswritingproofs} and \autoref{fig:proofscompleted}.  With Proof Previews, participants were able to complete the proofs at an overall average rate of 1.3 minutes per proof vs. 1.9 minutes without.  This was a statistically significant result (repeated measures ANOVA resulted in p=.033).

\begin{figure}[H]
  \center
  \includegraphics[width=.8\linewidth]{MinutesWritingProofs.jpg}
  \caption{The total number of minutes (capped at 15) each participant spent writing proofs, with and without Proof Previews.  Note that in the second set of 8, participants were required to use the Proof Preview feature, which had a significant effect (repeated measures ANOVA resulted in p=.005).}
...
\end{tabularx}
\caption{Satisfaction and preferences questionnaire average results for \textbf{proof interpretation using Proof Transitions}.  Values go from 1 to 9 (e.g. a 9 for a Terrible to Wonderful rating would mean completely wonderful).}
\end{table}
\label{tab:ptratings}
...
\end{tabularx}
\caption{Satisfaction and preferences questionnaire average results for \textbf{proof interpretation using the basic CoqEdit interface}.  Values go from 1 to 9 (e.g. a 9 for a Terrible to Wonderful rating would mean completely wonderful).}
\end{table}
...
\end{tabularx}
\caption{Satisfaction and preferences questionnaire average results for \textbf{theorem proving using Proof Previews}.  Values go from 1 to 9 (e.g. a 9 for a Terrible to Wonderful rating would mean completely wonderful).}
\end{table}
\label{tab:ppratings}
...
\end{tabularx}
\caption{Satisfaction and preferences questionnaire average results for \textbf{theorem proving using the basic CoqEdit interface}.  Values go from 1 to 9 (e.g. a 9 for a Terrible to Wonderful rating would mean completely wonderful).}
\end{table}
\label{tab:nonppratings}

%Comments for PP

Comments on Proof Previews versus basic CoqEdit included:
...
The results above are strong evidence for the beneficial effects on interactive theorem provers of two user interface features:  the proof tree visualization seen in Proof Transitions and the completion popup for potential tactics seen in Proof Previews.  The former appear to be a significant aid to understanding of the structure of the proof, and, specifically, answering the essential question of which goals are produced by which tactics.  The latter appear to increase the rate at which users are able to complete proofs (though the cause of this---e.g. how much is due to the ability to enter tactics with fewer keystrokes vs. how much is due to novice users not needing to look up tactics---remains an open question).  

Although on average, participants rated Proof Transitions worse than the basic CoqEdit interface, Proof Transitions does still appear to be a promising avenue for further research for several reasons.  First, a large minority of participants indicated that they actually did prefer Proof Transitions, and it was suggested that Proof Transitions might be useful for certain tasks (like presentations) that would be less suited to the basic CoqEdit.  There was also considerable bias against Proof Transitions going into the study because the video training did not spend much time on it and because, as a prototype, it did not allow editing of proofs.  

There is also a lot of apparent room for improvement with Proof Transitions.  The blinking animation could have been replaced by (as pointed out by Professor Aaron Stump) the ability to toggle through the matching text, and could have been made more salient perhaps by text that floats up to meet the matching text, or lines, or some other way of indicating correspondence.  It was likely, also, too slow for most users.  Other fixes might include auto-marking visited nodes, a minimap, and in-order node traversal.

The biggest problem with Proof Previews may be the Paradox of the Active User (see the next chapter)--that users are not necessarily aware of the ``Get Suggestions'' feature.  In an educational setting, an instructor may be able to compensate for this (e.g. by clearly demonstrating how to write proofs using the feature).  How to make users aware of the feature in other settings is an open question.
...
\begin{figure}[H]
  ...
  \caption{Example, stage 1, of syntax-tree highlighting and keyboard-card menu for propositional logic}
  \label{fig:pl0}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Example, stage 2, of syntax-tree highlighting and keyboard-card menu for propositional logic}
  \label{fig:pl1}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Example, stage 3, of syntax-tree highlighting and keyboard-card menu for propositional logic}
  \label{fig:pl2}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Example, stage 4, of syntax-tree highlighting and keyboard-card menu for propositional logic}
  \label{fig:pl3}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Example, stage 5, of syntax-tree highlighting and keyboard-card menu for propositional logic}
  \label{fig:pl4}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Example, stage 6, of syntax-tree highlighting and keyboard-card menu for propositional logic}
  \label{fig:pl5}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Example, stage 7, of syntax-tree highlighting and keyboard-card menu for propositional logic}
  \label{fig:pl6}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Example, stage 8, of syntax-tree highlighting and keyboard-card menu for propositional logic}
  \label{fig:pl07}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Example, stage 9, of syntax-tree highlighting and keyboard-card menu for propositional logic}
  \label{fig:pl8}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Example, stage 10, of syntax-tree highlighting and keyboard-card menu for propositional logic}
  \label{fig:pl9}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Example, stage 11, of syntax-tree highlighting and keyboard-card menu for propositional logic}
  \label{fig:pl10}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Example, stage 12, of syntax-tree highlighting and keyboard-card menu for propositional logic}
  \label{fig:pl11}
\end{figure}
...
A few other significant projects aiming to improve the presentation of machine-checked proofs include \textit{Proviola}, which allows users to move through a proof script displayed on a web page and view the results that Coq would produce \cite{tankink2012narrating}; an ``Interactive Derivation Viewer'' for visualizing derivations written in the TPTP language \cite{trac2007interactive}; the LOUI interface for the OMEGA proof assistant, featuring graphical visualization, term browsing, and natural language proof presentation \cite{siekmann1999loui}; and the Tecton system, featuring tree visualization combined with hypertext navigation of nodes \cite{kapur1994overview}.  Theoretical and methodological work has also been done and can be seen in \cite{eastaughffe1998support} (various design principles, e.g. ``the user interface should support several concurrent proof constructions''), \cite{bertot1998generic} (user interface architectural and design issues for generic ITPs including pretty printing and proof by pointing), \cite{luth1999functional} (presentation of prototype for visualization and direct manipulation), \cite{volker2004thoughts} (argument for reusable ITP user interface components, general HCI ideas that should be applied to ITPs), \cite{geuvers2009proof} (a more general discussion of theorem prover varieties and purposes), and \cite{luth2009user} (strengths and weaknesses of existing user interfaces, directions for future user interfaces).  Work aimed at making ITPs more suitable for novices and educational settings includes \cite{tsovaltzi2003approach} (student answer categorization, hints), \cite{meier2004adaptable} (adapting ITP GUIs to various educational settings), \cite{narboux2007graphical} (a graphical geometry tool that can do geometric proofs using Coq), \cite{billingsley2007student} (an interactive book for Number Theory on top of Isabelle, tested with students), \cite{dillinger2007acl2s} (an Eclipse-based interface for the ACL2 theorem prover, aimed at teaching new users how to use the prover), \cite{hendriks2010teaching} (ProofWeb and the generally positive experience using it in teaching), \cite{schreiner2009risc} (the RISC ProofNavigator, for program verification, developed with educational scenarios in mind) and \cite{pierce2010proofassist} (a presentation on pros and cons of using Coq to teach programming languages theory--``Bottom Line ... It works!'').

Perhaps the biggest gap in this literature, and the one the work in this dissertation starts to fill, is an effective long-term approach to improving interactive theorem prover user interfaces.  \cite{volker2004thoughts} presents a list of obstacles, including the fact that the academics and students who build ITP user interfaces have little incentive to perfect or maintain their software, and suggests ``reusable UI components and UIs'' as a way to mitigate these problems.  What I demonstrate in this dissertation is that testing of specific properties of theorem prover UI components and UIs is a viable path for academic research.  This suggests a way forward for incrementally-developed, and therefore better-engineered and maintained, user interfaces:  instead of the research cycle being 1) create entirely new (and probably limited and/or buggy and/or unmaintained) user interface 2) do a general evaluation (e.g. student questionnaire), 3) publish 4) repeat, the cycle might be 1) add (tentatively) a new UI component to an existing UI (which forces UI maintenance) 2) test specific properties of the new component 3) publish 4) repeat.

The space of new user interface components to test is very large.  In addition to previous work specifically on theorem prover user interfaces, it is important to consider more general human-computer interaction research and research on software development tools.  Starting with the latter, one can consider the data of \cite{murphy2006java} showing that the Eclipse IDE's ``rename,'' ``move,'' ``extract'', and ``inline'' refactoring commands are in fact used by many programmers and are frequently invoked via key bindings.  The data also shows that a large percentage of \textit{all} commands executed by developers using the IDE extensively were invoked via key bindings, and one of the top commands was ``content assist'', which suggests possible text to insert (similar to Proof Previews, though it does not show the effects of evaluating this text).  One may also note, here, the existence of various lines of work, e.g. the \textit{Frama-C} \cite{FramaC} project, aimed at integrating theorem proving and programming within IDEs, and the \textit{KeY} \cite{beckert2007verification} project (for ``Integrated Deductive Software Design'') which integrates graph visualization with theorem proving in the Eclipse IDE.
